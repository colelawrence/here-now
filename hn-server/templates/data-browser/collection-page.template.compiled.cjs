...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `${($$result.head += `${($$result.title = `<title>${escape(header.title)}</title>`, "")}<link rel="shortcut icon" href="/duckyhn.png" type="image/png">`, "")} <h1 class="svelte-1scqkfu"><div class="links svelte-1scqkfu">${each(header.links, ([collection_label, href]) => {
