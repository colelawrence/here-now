...
function generateColorSettings(colorConfig) {
  ...
  return {
    config: colorConfig,
    mvpTheme: (0, material_color_utilities_1.themeFromSourceColor)(
      (0, material_color_utilities_1.argbFromHex)(colorConfig.Primary.Hex),
      [...customColorsFromColorConfig(colorConfig)],
    ),
  };
}
exports.generateColorSettings = generateColorSettings;
async function generateColorSettingsFromHTMLImage(image, colorConfig) {
  const theme = await (0, material_color_utilities_1.themeFromImage)(image, [
    ...customColorsFromColorConfig(colorConfig),
  ]);
  // Get the theme from a hex color
  return {
